...
Why ?
=====
I found out that the local antivirus (McAfee) on my corporate workstation is configured to *NOT* analyze the content of a ZIP file, probably on the assumption that whenever this content is to be read or executed
it would normally first be extracted somewhere on the disk, and any bad stuff in it would get caught by the AV at that time.
...
How to use:
============
runfromzipinmemory.exe <zip_file> <exe_from_zip_file_to_be_executed>

Examples:
=========

c:\> runfromzipinmemory.exe mimikatz_trunk.zip x64/mimikatz.exe

c:\> runfromzipinmemory.exe meterpreter.zip meterpreter.exe
